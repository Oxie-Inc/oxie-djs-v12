...
module.exports = {
    run: async(message, args, client) => {
        if(message.member.voice.channel) {
            client.discordTogether.createTogetherCode(message.member.voice.channel.id, 'betrayal').then(async invite => {
                const embed = new MessageEmbed().setTitle('Betrayal').setDescription('Vous permet de jouer sur discord, avec ou sans vos amis !').setColor('RED');
    ...
    name: 'betrayal',
    help: {
        description: 'Permet de faire un battle royal'
    }
}