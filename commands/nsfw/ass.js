...
module.exports = {
    run: async(message, args) => {
        ...
        message.channel.send(loading).then(embed => {
            sa.get('https://nekobot.xyz/api/image').query({ type: 'ass' }).end((err, req) => {
                if(err) throw err;

                var e = new Discord.MessageEmbed().setDescription('Image ass').setImage(req.body.message).setTimestamp();
                embed.edit(e);
            })
        })
    },
